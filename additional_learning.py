...
dataset_path = r'dataset\230914-010822-train_test-vspo1-grayscale.npy'
...
#   画像ディレクトリ
img_dir = r'Vspo1\trimed_image'

#   出力先
out_dir = r'result\230914-024424_72x72_30epoch_vspo1-100-images-sgd'

#   入力
input_model_name = '230914-024424_model.h5'
#   出力
output_model_name = f'{timestamp}_model.h5'


history_path = f'{out_dir}/{timestamp}_history.csv'

intput_model_path = f'{out_dir}/{input_model_name}'
output_model_path = f'{out_dir}/{output_model_name}'
...
model.load_weights(intput_model_path)
...
print('fitting...')
# his = model.fit(X, y, epochs=epoch, shuffle=True, validation_split=0.2)
...
plt.savefig(f'{out_dir}/{timestamp}_loss.png')
plt.show()
plt.clf()
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.savefig(f'{out_dir}/{timestamp}_acc.png')
plt.show()
plt.clf()
